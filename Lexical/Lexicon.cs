...
namespace NLA_Tool.Lexical
{
    public class Lexicon
    {
        ...
        {
            var csvData = Csv.ParseCsv(filePath);

            var definitions = new List<Terminal>();
            foreach (var line in csvData)
            {
                var word = line[0];
                var definition = line[1];

                var d = new Terminal(word, StringtoLexicalCategory(definition));
        ...
        {
            var writer = new StreamWriter(_filePath, true); // Will append to the file.
